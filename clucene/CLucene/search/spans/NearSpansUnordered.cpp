...
bool NearSpansUnordered::next()
{
    ...
    {
        if( mintop()->next() )                     // trigger further scanning
    ...
    while( more )
    {
        ...
        if( mintop()->doc() != max->doc() )        // maintain list
        {             
            queueToList();
            queueStale = true;
        }
...
bool NearSpansUnordered::skipTo( int32_t target )
{
    ...
    {                                           // normal case
        while( more && mintop()->doc() < target )  // skip as needed
        {
            if( mintop()->skipTo( target ))
                queue->adjustTop();
            else 
                more = false;
        }
...
{
    return ( mintop()->doc() == max->doc() )
        && (( max->end() - mintop()->start() - totalLength ) <= slop );
}
