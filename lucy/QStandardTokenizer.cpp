...
CL_NS_USE(util)
  ...
  const TCHAR** QtokenImage = QtokenImageArray;
  ...
  ** length indicator and move the terminator back by one character. */
  ...
  QStandardTokenizer::QStandardTokenizer(BufferedReader* reader, bool deleteReader):
  ...
  QStandardTokenizer::~QStandardTokenizer() {
    _CLDELETE(rd);
    if ( this->deleteReader )
    	_CLDELETE(reader)
  }

  int QStandardTokenizer::readChar() {
  ...
  void QStandardTokenizer::unReadChar() {
    rd->UnGet();
    rdPos--;
  }

  inline Token* QStandardTokenizer::setToken(Token* t, StringBuffer* sb, TokenTypes tokenCode) {
    t->setStartOffset(tokenStart);
	  t->setEndOffset(tokenStart+sb->length());
      t->setType(QtokenImage[tokenCode]);
	  sb->getBuffer(); //null terminates the buffer
	  t->resetTermTextLen();
	  return t;
  }

  void QStandardTokenizer::reset(Reader* _input) {
	this->input = _input;
  ...
  Token* QStandardTokenizer::next(Token* t) {
    int ch=0;
  ...
  Token* QStandardTokenizer::ReadNumber(const TCHAR* previousNumber, const TCHAR prev,Token* t) {
  ...
  Token* QStandardTokenizer::ReadAlphaNum(const TCHAR prev, Token* t) {
    t->growBuffer(LUCENE_MAX_WORD_LEN+1);//make sure token can hold the next word
    StringBuffer str(t->termBuffer(),t->bufferLength(),true); //use stringbuffer to read data onto the termText
	  if (  str.len < LUCENE_MAX_WORD_LEN ){
		  str.appendChar(prev);
  ...
  Token* QStandardTokenizer::ReadCJK(const TCHAR prev, Token* t) {
    t->growBuffer(LUCENE_MAX_WORD_LEN+1);//make sure token can hold the next word
    StringBuffer str(t->termBuffer(),t->bufferLength(),true); //use stringbuffer to read data onto the termText
	  if ( str.len < LUCENE_MAX_WORD_LEN ){
		  str.appendChar(prev);
  ...
  Token* QStandardTokenizer::ReadDotted(StringBuffer* _str, TokenTypes forcedType, Token* t) {
  ...
  Token* QStandardTokenizer::ReadApostrophe(StringBuffer* _str, Token* t) {
    StringBuffer& str=*_str;
  ...
  Token* QStandardTokenizer::ReadAt(StringBuffer* str, Token* t) {
  ...
  Token* QStandardTokenizer::ReadCompany(StringBuffer* _str, Token* t) {
    StringBuffer& str = *_str;
    const int32_t specialCharPos = rdPos;
    int ch=0;
